...
class DirectChatViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return DirectChat.objects.filter(participants=self.request.user)
    ...
    def messages(self, request, pk=None):
        ...
        messages = DirectMessage.objects.filter(
            Q(sender=self.request.user, receiver__in=chat.participants.all()) |
            Q(receiver=self.request.user, sender__in=chat.participants.all())
        ).order_by('timestamp')
