...
class MeetingViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            for member in study_group.members.all():
                if member != request.user:  # Don't notify the creator
                    send_notification(
                        member,
                        f"New meeting '{meeting.title}' has been created in {study_group.name}"
                    )
    ...
    def availability(self, request, pk=None):
        try:
            ...
            return Response(
                {'error': 'An error occurred while processing availability'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
